    ...
    void HL2ResearchMode::DepthSensorLoop(HL2ResearchMode* pHL2ResearchMode)
    {
        ...
        try 
        {
            ...
            while (pHL2ResearchMode->m_depthSensorLoopStarted)
            {
                ...
                // TODO: Make the bounds equal to the ROI, remove ROI conditions   
                for (UINT i = pHL2ResearchMode->depthCamRoi.kRowLower * resolution.Height; i < pHL2ResearchMode->depthCamRoi.kRowUpper * resolution.Height; i++)
                {
                    for (UINT j = pHL2ResearchMode->depthCamRoi.kColLower * resolution.Width; j < pHL2ResearchMode->depthCamRoi.kColUpper * resolution.Width; j++)
                    {
                        ...
                        if (pHL2ResearchMode->m_reconstructShortThrowPointCloud)
                        {
                            ...
                            {
                                // on the condition that it is in the ROI, but at an invalid clip range, still add a NAN point. this is less efficient, but allows for an organised cloud 
                                pointCloud.push_back(NAN); // x
                                pointCloud.push_back(NAN); // y
                                pointCloud.push_back(NAN); // z
                            }
                        }
                        // TODO: Remove everything that isnt to do with short ab image texture
